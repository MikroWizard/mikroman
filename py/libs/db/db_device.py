...
class Devices(BaseModel):
    ...
    # PAM expansion columns (Task 9)
    device_type = TextField(null=True)  # FK → device_brands.brand
    device_model = TextField(null=True)  # hardware model (from router_type backfill)
    template_id = IntegerField(null=True)  # FK → device_templates.id
    credential_migrated = BooleanField(
        default=False
    )  # set True by migrate_credentials.py
...
def update_devices_firmware_status(data):
    ...
    query = Devices.insert_many(data).on_conflict(
        conflict_target=Devices.id,
        update={
            Devices.update_availble: EXCLUDED.update_availble,
            Devices.upgrade_availble: EXCLUDED.upgrade_availble,
            Devices.current_firmware: EXCLUDED.current_firmware,
            Devices.arch: EXCLUDED.arch,
        },
    )
    query.execute()
    return True
